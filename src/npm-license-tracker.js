...
getExtendedJson = function (path, json, packages) {
  var updatedpackages = Object.keys(json).map((x) => {
    var info = json[x];
    var repoName = info.repository
      ? info.repository.replace("git+", "")
      : _noInfoFound;
    return {
      ...
      dependencyType:
        packages.filter((pkg) => pkg.label === x).length > 0
          ? "immediate"
          : "transitive",
    };
  });
...
generateCsvFile = function (path, packages) {
  ...
  const csv = parse(updatedPackages, opts);
...
getDependencies = function (pDependencies, pDevDependencies) {
  const dependencies = Object.keys(pDependencies).map((x) => {
    const label =
      pDependencies[x].indexOf("^") >= 0
        ? pDependencies[x].slice(1)
        : pDependencies[x];
    return {
      name: x,
  ...
  const devDependencies = Object.keys(pDevDependencies).map((x) => {
    const label =
      pDevDependencies[x].indexOf("^") >= 0
        ? pDevDependencies[x].slice(1)
        : pDevDependencies[x];
    return {
      name: x,
